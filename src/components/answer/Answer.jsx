...
function Answer({ prompt, response, favoriteCompanies, setFavoriteCompanies, answerLoading }) {
  ...
  const fetchNewsForCompany = async (companyName) => {
    try {
      const res = await axios.post(`https://https://syukatu-app-backend.onrender.com//api/news`, { companyName });
      console.log("res", res);
  ...
  const fetchFavoriteCompanies = async () => {
    try {
      const res = await axios.get(`https://https://syukatu-app-backend.onrender.com//api/users/${user._id}/fetchFavoriteCompanies`);
  ...
  const addToFavoriteCompanies = async () => {
    ...
    try {
      await axios.put(`https://https://syukatu-app-backend.onrender.com//api/users/${user._id}/addToFavoriteCompanies`, { userId: user._id, addFavoriteCompanyContent });
    } catch (err) {
      console.log(err);
    }
  ...
  const deleteFromFavoriteCompanies = async (deleteCompanyName, deletCompanyAbout) => {
    ...
    try {
      await axios.delete(`https://https://syukatu-app-backend.onrender.com//api/users/${user._id}/deleteFromFavoriteCompanies`, {
        data: {
          userId: user._id,
